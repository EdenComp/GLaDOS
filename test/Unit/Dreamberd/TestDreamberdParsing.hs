...
testParseFunction =
    TestList
        ...
        , TestCase (assertEqual "parseFunction with multi-content" (Right ("", [Function "foo" ["bar"] [Call "int" [Identifier "a", Number 42], Call "int" [Identifier "b", Identifier "bar"], Return (Identifier "bar")]])) (parseFunction "foo ( bar ) {\nint a = 42; int b = bar; return bar;\n}" []))
...
testParseCondition =
    TestList
        ...
        , TestCase (assertEqual "parseCondition with elif and else" (Right ("", [If (Boolean True) [Call "str" [Identifier "a", String "test"], Call "=" [Identifier "a", String "other"]] [If (Boolean True) [Return (Number 2)] [Return (Number 3)]]])) (parseCondition "(true) {str a = \"test\"; a = \"other\";} elif (true) {return 2;} else {return 3;}" []))
        , TestCase (assertEqual "parseCondition with elif, else and spaces" (Right ("", [If (Boolean True) [Return (Number 1)] [If (Boolean True) [Call "bool" [Identifier "c", Boolean False]] [Return (Number 3)]]])) (parseCondition " (true) {return 1;} elif (true) {bool c =  false;} else {return 3;}" []))
...
testParseDreamberd =
    TestList
        [ TestCase (assertEqual "parseDreamberd basic assign var" (Right [Call "int" [Identifier "a", Number 1]]) (parseDreamberd "int a = 1;" []))
        , TestCase (assertEqual "parseDreamberd reasign var" (Right [Call "int" [Identifier "a", Number 1], Call "=" [Identifier "a", Number 2]]) (parseDreamberd "int a = 1;a = 2;" []))
        ...
        , TestCase (assertEqual "parseDreamberd basic loop" (Right [Loop (Boolean True) [Call "int" [Identifier "b", Number 42]] Nothing Nothing]) (parseDreamberd "while (true) {int b = 42;}" []))
