...
parseValue (c : bytes) = case fromEnum c of
    ...
    0x16 -> parseInt bytes >>= \(len, rest) ->
        let (insts, rest') = splitAt len rest in parseInstructions insts >>= \func -> if length rest < len then Left "Wrong lambda body length" else Right (Lambda func, rest')
