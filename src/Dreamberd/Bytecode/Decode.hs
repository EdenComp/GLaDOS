...
parseInt (b1:b2:b3:b4:rest) = Right (val, rest)
    where val = fromEnum b1 * 256 * 256 * 256 + fromEnum b2 * 256 * 256 + fromEnum b3 * 256 + fromEnum b4
...
parseString bytes = case parseInt bytes of
    ...
    Right (val, rest) -> if length rest < val then Left "Wrong string length" else Right (str, rest')
        where (str, rest') = splitAt val rest
...
parseEnvValue (c:bytes) | fromEnum c == 0x30 = case parseInt bytes of
    Left err -> Left err
    Right (len, rest) -> case parseInstructions insts of
        ...
        where (insts, rest') = splitAt len rest
