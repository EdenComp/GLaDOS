...
prettyPrintNodes (Loop test body initNode updateNode : xs) sp = replicate sp ' ' ++ "Loop (\n" ++ prettyPrintNodes [test] (sp + 4) ++ "init:\n" ++ prettyPrintNodes (maybeToList initNode) (sp + 4) ++ prettyPrintNodes [test] (sp + 4) ++ "condition:\n" ++ prettyPrintNodes [test] (sp + 4) ++ prettyPrintNodes [test] (sp + 4) ++ "update:\n" ++ prettyPrintNodes (maybeToList updateNode) (sp + 4) ++ prettyPrintNodes [test] (sp + 4) ++ "body:\n" ++ prettyPrintNodes body (sp + 4) ++ replicate sp ' ' ++ ")\n" ++ prettyPrintNodes xs sp
