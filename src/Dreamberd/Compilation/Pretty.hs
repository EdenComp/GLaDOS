...
prettyPrintNodes (Lambda params body : xs) sp = replicate sp ' ' ++ "Lambda (" ++ intercalate ", " params ++ ") {\n" ++ prettyPrintNodes body (sp + 2) ++ replicate sp ' ' ++ "}\n" ++ prettyPrintNodes xs sp
prettyPrintNodes (Call name params : xs) sp = replicate sp ' ' ++ "Call\n" ++ replicate (sp + 2) ' ' ++ prettyPrintNodes [name] (sp + 4) ++ replicate (sp + 2) ' ' ++ "(\n" ++ prettyPrintNodes params (sp + 2) ++ replicate sp ' ' ++ ")\n" ++ prettyPrintNodes xs sp
...
prettyPrintNodes (Loop test body initNode updateNode : xs) sp = replicate sp ' ' ++ "Loop (\n" ++ replicate (sp + 2) ' ' ++ "init:\n" ++ prettyPrintNodes (maybeToList initNode) (sp + 4) ++ replicate (sp + 2) ' ' ++ "condition:\n" ++ prettyPrintNodes [test] (sp + 4) ++ replicate (sp + 2) ' ' ++ "update:\n" ++ prettyPrintNodes (maybeToList updateNode) (sp + 4) ++ replicate (sp + 2) ' ' ++ "body:\n" ++ prettyPrintNodes body (sp + 4) ++ replicate sp ' ' ++ ")\n" ++ prettyPrintNodes xs sp
prettyPrintNodes (Scope scope : xs) sp = replicate sp ' ' ++ "List [\n" ++ prettyPrintNodes scope (sp + 2) ++ replicate sp ' ' ++ "]\n" ++ prettyPrintNodes xs sp
